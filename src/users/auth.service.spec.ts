...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    // Create a fake copy ot the users service
    const users: User[] = [];
  ...
  it('creates a new user with salted ans hashed password', async () => {
    const user = await service.signup('asadf@asdf.com', 'asdfdf');

    expect(user.password).not.toEqual('asdfdf');
  ...
  it('throws if an invalid password is provided', async () => {
    ...
    await expect(service.signin('asd@asd.com', 'asdf')).rejects.toThrow(BadRequestException);
  });

  it('returns a user if correct password is provided', async () => {
    await service.signup('asd@asd.com', 'asdf');
    const user = await service.signin('asd@asd.com', 'asdf');
